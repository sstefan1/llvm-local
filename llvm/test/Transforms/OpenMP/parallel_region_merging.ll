...
; RUN: opt -S -attributor -attributor-enable-all-abstract-attributes -openmpopt -openmp-opt-enable-merging -enable-new-pm=0 < %s | FileCheck %s
; RUN: opt -S -aa-pipeline= -passes='attributor,cgscc(openmpopt)' -attributor-enable-all-abstract-attributes -openmp-opt-enable-merging  < %s | FileCheck %s
; #include <omp.h>
; void foo();
; void use(int);
; void usef(float);
