; RUN: opt -S -pass-remarks=openmp-opt -attributor -attributor-enable-all-abstract-attributes -openmpopt -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt -S -pass-remarks=openmp-opt -passes='attributor,cgscc(openmpopt)' -attributor-enable-all-abstract-attributes -disable-output < %s 2>&1 | FileCheck %s
